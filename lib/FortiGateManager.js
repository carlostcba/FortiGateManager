...
class FortiGateManager {
  ...
  loadConfig() {
    this.config = {
      connection: {
        hostname: process.env.FORTIGATE_HOST || '10.0.10.1',
        username: process.env.FORTIGATE_USERNAME || 'admin',
        password: process.env.FORTIGATE_PASSWORD || 'password',
        port: parseInt(process.env.FORTIGATE_PORT) || 22,
        timeout: parseInt(process.env.FORTIGATE_TIMEOUT) || 20000
      }
    };
